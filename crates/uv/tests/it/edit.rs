...
/// In auth-mode "always", the normal authentication flow should still work.
#[test]
fn add_auth_mode_always_with_credentials() -> Result<()> {
    let context = TestContext::new("3.12");
...
/// In auth-mode "always", unauthenticated requests to a registry that
/// doesn't require credentials will fail.
#[test]
fn add_auth_mode_always_without_credentials() -> Result<()> {
    let context = TestContext::new("3.12");
...
/// In auth-mode "never", even if the correct credentials are supplied,
/// no authenticated requests will be allowed.
#[test]
fn add_auth_mode_never_with_credentials() -> Result<()> {
    let context = TestContext::new("3.12");
...
/// In auth-mode "never", the normal flow for unauthenticated requests should
/// still work.
#[test]
fn add_auth_mode_never_without_credentials() -> Result<()> {
    let context = TestContext::new("3.12");
